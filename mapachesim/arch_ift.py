...
class Mips_IFT(IsaDefinition):
    ...
    def split_mem(self):
        ...
        self.ift_start_address = int(self.ift_ratio*data_space/partitions)
        self.ift_start_address += self.text_start_address
        #Normalize to data alignment
        self.ift_start_address -= self.ift_start_address % 4
    ...
    def get_ift_addr(self, addr):
        ...
        return self.ift_start_address + int(offset/self.ift_ratio)
    ...
    def print_mem(self):
        ...
        buf = '          '
        print()
        for addr in self.mem_locations:
            ...
            data_str = [' '.join(chunk) for chunk in _chunk_list(data, 4)]
    ...
    def instruction_lw(self, ifield):
        ...
        self.invalid_when(addr >= self.ift_start_address, \
            'Cannot directly load IFT information')
    ...
    def instruction_sw(self, ifield):
        ...
        self.invalid_when(addr >= self.ift_start_address, \
            'Address encroaching on memory reserved for IFT')
